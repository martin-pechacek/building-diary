...
class DiaryEntryFacadeImplTest {
    ...
    class Photos {
        ...
        void shouldUploadPhoto() {
            MultipartFile file = new MockMultipartFile("photo", "test.jpg", "image/jpeg", new byte[]{1, 2, 3});
            String description = "Test photo";
            PhotoDto photoDto = new PhotoDto(PHOTO_ID, "test.jpg", "image/jpeg", 3L, description, Instant.now());

            when(photoService.upload(ENTRY_ID, file, description)).thenReturn(photoDto);

            PhotoDto result = diaryEntryFacade.uploadPhoto(ENTRY_ID, file, description);

            assertThat(result.id()).isEqualTo(PHOTO_ID);
            verify(photoService).upload(ENTRY_ID, file, description);
        }
        ...
        void shouldGetPhoto() {
            ...
            Resource result = diaryEntryFacade.getPhoto(PHOTO_ID);

            assertThat(result).isEqualTo(resource);
            verify(photoService).getFile(PHOTO_ID);
        }

        @Test
        void shouldDeletePhoto() {
            diaryEntryFacade.deletePhoto(PHOTO_ID);
